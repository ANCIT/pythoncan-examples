...
class CANoe:
    ...
    def load_testsetup(self, tseName):
        if (self.application != None and len(tseName) > 0):
            ...
            if os.path.isfile(tseName) and (os.path.splitext(tseName)[1] == ".tse"):
                ...
                testSetup.TestEnvironments.Add(tseName)
    ...
    def start_testmodule(self):
        if (self.application.Configuration.TestSetup.TestEnvironments.Item(1) != None):
            # Fetch the 1st TestEnvironment and get the TestModule
            # Execute it
            testSetup = self.application.Configuration.TestSetup
            testEnvironment = testSetup.TestEnvironments.Item(1)
            self.testEnvironment = win32com.client.CastTo(testEnvironment, "ITestEnvironment2")
            test_module = self.testEnvironment.TestModules.Item(1)
            test_module.Start()
        else:
            raise RuntimeError("Test Specification is missing,unable to start testing")
