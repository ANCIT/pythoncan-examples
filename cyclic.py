...
def simple_periodic_send(bus):
    ...
    task = bus.send_periodic(msg, 0.50)
...
def limited_periodic_send(bus):
    ...
    task = bus.send_periodic(msg, 0.50, 1, store_task=False)
...
def main():
    ...
    with can.Bus(bustype='vector', app_name='CANoe', channel=0, bitrate=500000) as bus:
        bus.send(reset_msg)
