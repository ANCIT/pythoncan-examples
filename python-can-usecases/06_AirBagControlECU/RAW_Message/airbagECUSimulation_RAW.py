...
def on_Message():
	while True:
		...
		if response.arbitration_id == seatbeltMsgID:
			seatbeltWorn = (bool(response.data[0] & (1<<7)))  #7th Bit of 0th Byte
		...
		if response.arbitration_id == VehicleMotionMsgID:
			crashDetected = (bool(response.data[0] & (1<<3)))  #3rd Bit of 0th Byte
			engineRunning = (bool(response.data[0] & (1<<2)))  #2nd Bit of 0th Byte
